...
if len(args) < 2:
  ...
  m_id = session.query(
      Markets.market_id).filter_by(market_predictit_id=int(args[1])).scalar()
  if m_id is None:
    print()
    print("Market: " + str(args[1]) + " not found.")
  else:
    m_name = session.query(
        Markets.market_name).filter_by(market_id=m_id).scalar()
    c_ids = session.query(Contracts.contract_id).filter_by(market_id=m_id).all()
    for a in c_ids:
      fileout = open(str(args[1]) + '-' + str(a[0]) + '.csv', 'w')
      outcsv = csv.writer(fileout)
      records = session.query(Prices).filter_by(contract_id=a[0]).all()
      outcsv.writerow(
          [column.name.encode('utf-8') for column in Prices.__mapper__.columns])
      [
          outcsv.writerow([
              getattr(curr, column.name) for column in Prices.__mapper__.columns
          ]) for curr in records
      ]
      fileout.close()
    print('Market: ' + m_name + ' found and dumped to ' + str(len(c_ids)) +
          ' csv files.')
